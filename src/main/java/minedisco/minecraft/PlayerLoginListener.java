...
public class PlayerLoginListener implements Listener {
    ...
    public void playerLogin(PlayerLoginEvent event) {
        ...
        if (this.whitelistHandler.checkIfWhitelistContains(uuid) && this.whitelistHandler.checkIfAuthDone(uuid)) {
            ...
            } else if (!this.whitelistHandler.checkIfConfirmedIpForUser(uuid, event.getAddress().getHostAddress())) {
                String authToken = this.whitelistHandler.startIPAuthProcess(uuid, event.getAddress().getHostAddress());
            ...
            String authToken = this.whitelistHandler.startAuthProcess(event.getPlayer().getName(), uuid, event.getAddress().getHostAddress());
